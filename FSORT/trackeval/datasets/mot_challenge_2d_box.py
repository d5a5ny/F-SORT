...
class MotChallenge2DBox(_BaseDataset):
    ...
    def get_default_dataset_config():
        ...
        default_config = {
            ...
            'CLASSES_TO_EVAL': ['plane'],  # Valid: ['pedestrian']
    ...
    def __init__(self, config=None):
        ...
        # Get classes to eval
        self.valid_classes = ['plane']
        self.class_list = [cls.lower() if cls.lower() in self.valid_classes else None
                           for cls in self.config['CLASSES_TO_EVAL']]
        if not all(self.class_list):
            raise TrackEvalException('Attempted to evaluate an invalid class. Only plane class is valid.')
        self.class_name_to_class_id = {'plane': 1}
    ...
    def _load_raw_file(self, tracker, seq, is_gt):
        ...
        extra_time_keys = [x for x in read_data.keys() if x not in [str(t+1) for t in range(num_timesteps)]]   # I modified from t+1 to only t in the str(t)
    ...
    def get_preprocessed_seq_data(self, raw_data, cls):
        ...
        for t in range(raw_data['num_timesteps']):
            ...
            # Evaluation is ONLY valid for plane class
            if len(tracker_classes) > 0 and np.max(tracker_classes) > 1:
                raise TrackEvalException(
                    'Evaluation is only valid for plane class. Non plane class (%i) found in sequence %s at '
